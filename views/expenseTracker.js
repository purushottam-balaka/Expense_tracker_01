...
window.addEventListener('DOMContentLoaded',async()=>{
    ...
    await axios.get(`http://localhost:8080/get_expenses?page=${page}&&rows=${row}`,{headers:{'Authorization':token}})
...
function showExpeneseOnScreen(exp){
    ...
    editButton.onclick=async()=>{
        try{
            const token=localStorage.getItem('token');
            const user=await axios.get(`http://localhost:8080/edit_expense/${editButton.id}`,{headers:{'Authorization':token}})
            ...
            await axios.delete(`http://localhost:8080/delete_expense/${editButton.id}`,{headers:{'Authorization':token}})    
    ...
    delButton.onclick=async()=>{
        ...
        const token=localStorage.getItem('token')
        parentEle.removeChild(chiledEle)
        await axios.delete(`http://localhost:8080/delete_expense/${delButton.id}`,{headers:{'Authorization':token}})
         .then((resp)=>{
...
async function getProdcuts(page){
    ...
    try{
        ...
        const token=localStorage.getItem('token');
         await axios.post('http://localhost:8080/add_expense',expenseDetails,{headers: {'Authorization':token}})
         .then((res)=>{
            window.location.reload();
         }).catch((err)=>{
            console.log(err)
    ...
    async function downloadFile(e){
       axios.get('http://localhost:8080/download',{headers:{Authorization:token}})
       .then((resp)=>{
        if(resp.status===200){
